...
void FST_Analysis_V2(char* filename, int board)
{
  ...
  int nRows = (int)round((float)nPointsTot / 2.);
  ...
  */
  ...
  const float maxNph = 400.e3;

  const int nPointsLarge = 19;   // number of scan points was originally 8
  const int nPointsSmall = 63;   // number of scan points was originally 8
  const int nPointsTot = nPointsLarge * nPointsSmall;

  const int startPointLarge = 11;  // start trigger set //changed from 4
  ...
  for (int ipoint = 0; ipoint < nPointsLarge; ipoint++){
	...
	sprintf(selection,"widthsetLarge==%d&&widthsetSmall==%d&&amp2<-10&&width2<40&&width2>2",trigSetLarge, trigSetSmall); // widthset is the trigger width set and pulseWidth is the pulse width.
      ...
      */
      ...
      float SmallWval=(((trigSetSmallf*100)/63)/100);
  ...
  */
  ...
  //c2->SetLogy(0);
  //c2->SetLogx();
  TH2F* PWvsNPH = new TH2F("PWvsNPH",titlename,1000,1,maxNph,100,0.,pwMax+5.);
  ...
  //  TGraphErrors* nphVSpw = new TGraphErrors(nPointsTot,pw,nphPMT,erpw,ernphPMT); //number of photons VS pulse width
  TGraphErrors* nphVSpw = new TGraphErrors(nPointsTot,pw,nphPMT); //number of photons VS pulse width
  sprintf(titlename,"PB%02d",board);
  TCanvas *c3 = new TCanvas("c3", "c3",10,65,700,502);
  c3->SetGridy();
  //c3->SetLogy(0);
  //c3->SetLogx();
  TH2F* NPHvsPW = new TH2F("NPHvsPW",titlename,100,1.,pwMax+5.,1000,1,maxNph);
  NPHvsPW->SetStats(kFALSE);
  NPHvsPW->GetYaxis()->SetTitle("Number of photons per pulse");
  ...
  TH2F* TWvsNPH = new TH2F("TWvsNPH",titlename,1000,1,maxNph,65,0,65);
  ...
  TGraphErrors* nphVStw = new TGraphErrors(nPointsTot,trigW,nphPMT,ertrigW,ernphPMT);  //nph VS Trigger Width
  sprintf(titlename,"PB%02d",board);
  TCanvas *c5 = new TCanvas("c5","c5",10,65,700,500);
  c5->SetGridy();
  //c5->SetLogx();
  //c5->Range(1,0,maxNph,65);
  TH2F* NPHvsTW = new TH2F("NPHvsTW",titlename,65,0,65,1000,1,maxNph);
  ...
  /*
  TGraphErrors* twVSlnph = new TGraphErrors(nPointsTot,lnphPMT,trigW,erlnphPMT,ertrigW); //Natural Log of nph VS Trigger Width
  sprintf(titlename,"PB%02d",board);
  TCanvas *c6 = new TCanvas("c6","c6",10,65,700,500);
  c6->SetGridy();
  c6->SetLogx();
  //c6->Range(1,0,maxNph,65);
  ...
  */
  ...
  TGraphErrors* nphVSwidthsetL = new TGraphErrors(nPointsTot,widthsetlarge,nphPMT,0,ernphPMT);  //nph VS Trigger Width
  sprintf(titlename,"PB%02d",board);
  TCanvas *c7 = new TCanvas("c7","c7",10,65,700,500);
  c7->SetGridy();
  //c7->SetLog();
  //c7->Range(1,0,maxNph,65);
  TH2F* NPHvsWIDTHSETL = new TH2F("NPHvsTW",titlename,24,8,20,1000,1,maxNph);
  ...
  TGraphErrors* nphVSwidthsetS = new TGraphErrors(nPointsTot,widthsetdec,nphPMT,0,ernphPMT);  //nph VS Trigger Width
  sprintf(titlename,"PB%02d",board);
  TCanvas *c8 = new TCanvas("c8","c8",10,65,700,500);
  c8->SetGridy();
  //c8->SetLog();
  //c8->Range(1,0,maxNph,65);
  TH2F* NPHvsWIDTHSETS = new TH2F("NPHvsTW",titlename,100,10,20,1000,1,maxNph);
